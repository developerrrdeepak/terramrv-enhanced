  ...
  try {
    ...
    const getGEEAccessToken = async () => {
      try {
        ...
        const { client_email: clientEmail, private_key: privateKey, project_id: projectId } = json;
    ...
    const processNdviJobs = async () => {
      try {
        const jobs = await db.getDb().collection('mrv_jobs').find({ status: 'queued' }).toArray();
        ...
        for (const job of jobs) {
          ...
          // Mark as processing
          await db.getDb().collection('mrv_jobs').updateOne({ _id: job._id }, { $set: { status: 'processing', startedAt: new Date() } });
          ...
          if (geeAuth && geeAuth.access_token) {
            console.log('GEE auth succeeded, would run NDVI task using project', geeAuth.projectId || 'unknown');
            // TODO: call Earth Engine APIs to compute NDVI and export results
            // For demo, produce deterministic mock based on job._id
            const seed = job._id.toString().split('').reduce((s, c) => s + c.charCodeAt(0), 0);
            const ndviMean = ((seed % 30) + 45) / 100; // e.g., 0.45 - 0.75
            result = { ndviMean, timeseries: Array.from({ length: 6 }).map((_, i) => ({ date: new Date(Date.now() - (5 - i) * 30 * 24 * 3600 * 1000).toISOString().split('T')[0], value: Math.max(0, ndviMean + (Math.random() - 0.5) * 0.05) })) };
          } else {
            console.log('GEE auth unavailable, using mock NDVI result');
            const ndviMean = 0.52 + Math.random() * 0.15;
            result = { ndviMean, timeseries: Array.from({ length: 6 }).map((_, i) => ({ date: new Date(Date.now() - (5 - i) * 30 * 24 * 3600 * 1000).toISOString().split('T')[0], value: Math.max(0, ndviMean + (Math.random() - 0.5) * 0.08) })) };
          }

          // Store result and mark job done
          await db.getDb().collection('mrv_jobs').updateOne({ _id: job._id }, { $set: { status: 'done', result, finishedAt: new Date() } });

          // Audit log
          await db.getDb().collection('audit_logs').insertOne({ type: 'ndvi_completed', jobId: job._id, resultSummary: { ndviMean: result.ndviMean }, timestamp: new Date() });

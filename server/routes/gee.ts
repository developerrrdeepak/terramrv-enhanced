...
export const validateGEEKey: RequestHandler = async (req, res) => {
  try {
    ...
    if (!url) return res.status(400).json({ success: false, message: 'GEE service account URL not configured' });

    const r = await fetch(url);
    if (!r.ok) return res.status(502).json({ success: false, message: 'Failed to fetch GEE JSON' });

    const json = await r.json();
    // Basic validation: check for client_email and private_key
    if (!json.client_email || !json.private_key) {
      return res.status(400).json({ success: false, message: 'Invalid GEE service account JSON' });
    }

    res.json({ success: true, message: 'GEE service account JSON accessible', client_email: json.client_email });
  } catch (error) {
...
export const queueNdviJob: RequestHandler = async (req, res) => {
  try {
    ...
    const { area, description } = req.body as { area?: any; description?: string };
    if (!area) return res.status(400).json({ success: false, message: 'area required' });

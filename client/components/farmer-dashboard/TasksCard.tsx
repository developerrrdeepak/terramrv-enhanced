...
export default function TasksCard() {
  ...
  return (
    <Card className="rounded-2xl shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><ListChecks className="w-5 h-5 text-[#4CAF50]"/> Tasks & Reminders</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
          {tasks.map((task) => (
            <div key={task.id} className="flex items-center justify-between bg-[#F9F9F9] p-2 rounded-lg">
              <div>
              ...
              <button onClick={() => toggle(task.id)} className={`px-3 py-1 rounded-md ${task.done ? 'bg-green-100 text-green-700' : 'bg-white border'}`}>{task.done ? 'Done' : 'Mark'}</button>
            </div>
          ))}

          <div className="flex gap-2">
            <Input value={val} onChange={(e) => setVal(e.target.value)} placeholder="Add task" />
            <Button onClick={add} className="bg-[#4CAF50]">Add</Button>
          </div>
        </div>
      </CardContent>
    </Card>
